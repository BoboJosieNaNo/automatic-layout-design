...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 10 19:35:42 2023
...
def visualize(width, height, item1, item2, No):
    ...
    for item_name, item_list in item1.items():
    ...
    position_note = '\n'.join([f'{key}: {value}' for key, value in item2.items()])
    fig.text(0.1, 0.02, position_note, fontsize=8, ha='left', va='top')
...
while not inputfile.empty:
    ...
    #set order quantity
    qty_uqe_logo={i: inputfile.loc[i, 'Required Qty'] for i in range(inputfile.shape[0])}
    ...
    if __name__ == '__main__':
        ...
        filled_size=0
        for rect in rects:
            filled_size=filled_size+rect.width*rect.height
            if filled_size>2*(container_width*container_height):
                break
            else:
                processing_rects.append(rect)

        #sort by area
        rects_sorted = sorted(processing_rects, key=lambda x: x.area(), reverse=True)

        # Initialize the Guillotine algorithm
        guillotine = Guillotine(container_width, container_height)

        # Place the rectangles into the container
        for rect in rects_sorted:
            success = guillotine.place_rect(rect)
            if not success: #not in used_rectangles
                processing_rects.remove(rect)
        ...
        for rect in processing_rects:
            PackingResult.append((rect.index,[rect.x,rect.y,rect.width,rect.height]))
        ...
        used_space=0
        for rect in processing_rects:
            used_space=used_space+rect.width*rect.height
        wasted_space=1-used_space/(container_width*container_height)
        print("%wasted space = "+str(wasted_space))

        # #up
        for rect in processing_rects:
            outputfile.loc[inputfile.loc[rect.index[0],'No']-1, '# Up'] = number_up(processing_rects,rect.index[0])        
        
        #amount of carrier sheet
        SheetAmount=0
        for rect in processing_rects:
            s=math.ceil(qty_uqe_logo[rect.index[0]]/number_up(processing_rects,rect.index[0]))
            if s>SheetAmount:
                SheetAmount=s
            else:
                continue
        ...
        for rect in processing_rects:
            outputfile.loc[inputfile.loc[rect.index[0],'No']-1, 'Sheets'] = SheetAmount
            
        print("total amount of sheets="+str(SheetAmount))

        #wasted unneeded transfer
        for rect in processing_rects:
            WastedTransfer=SheetAmount*number_up(processing_rects,rect.index[0])-qty_uqe_logo[rect.index[0]]
            outputfile.loc[inputfile.loc[rect.index[0],'No']-1, 'Waste'] = WastedTransfer
            print(rect.index,"Rectangle("+str(rect.width)+", "+str(rect.height)+"): "+str(WastedTransfer))
        ...
        LogoNameValue_dict = {}
        for item in PackingResult:
            i = item[0][0]
            logo_name = LogoName_dict[i]
            rect = item[1]
            if logo_name in LogoNameValue_dict:
                LogoNameValue_dict[logo_name].append(rect)
            else:
                LogoNameValue_dict[logo_name] = [rect]

        Note_LogoNameValue_dict = {}
        for item in PackingResult:
            i = item[0][0]
            logo_name = LogoName_dict[i]
            if logo_name in Note_LogoNameValue_dict:
                Note_LogoNameValue_dict[logo_name].append((math.ceil(item[1][0]),math.ceil(item[1][1])))
            else:
                Note_LogoNameValue_dict[logo_name] = [(math.ceil(item[1][0]),math.ceil(item[1][1]))]
                        
        visualize(17.83, 11.27, LogoNameValue_dict, Note_LogoNameValue_dict, LayoutPDF)
